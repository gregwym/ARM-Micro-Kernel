...
int stopCheck(track_node *cur_node, int ahead, char *switch_table, int stop_distance, track_node *stop_node, track_node **route, int check_point, int com2_tid) {
	...
	if (stop_node == NULL) {
		...
		for (; distance < (stop_distance + ahead) && !hit_exit; check_point++) {
