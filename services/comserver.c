...
void comSendNotifier() {
	...
	while (1) {
		result = Send(serverInfo.server_tid, &send, sizeof(char), reply, sizeof(char));
		assert(result == sizeof(char), "Send Notifier got invalid reply size");

		result = AwaitEvent(eventId , reply, sizeof(char));
...
void comReceiveNotifier() {
	...
	while (1) {
		result = AwaitEvent(eventId , send, sizeof(char));
		assert(result == 0, "Receive Notifier wait event failed");

		result = Send(serverInfo.server_tid, send, 2, NULL, 0);
...
int Getc(int channel) {
	...
	char reply[2];
	if (channel == 1) {
		char server_name[] = COM1_REG_NAME;
		int com1_server_tid = WhoIs(server_name);
		...
		char server_name[] = COM2_REG_NAME;
		int com2_server_tid = WhoIs(server_name);
...
void comserver() {
	...
	while(1) {
		...
		if (send_notifier_is_waiting && send_buffer.current_size > 0) {
			...
			Reply(send_notifier_tid, reply, sizeof(char));
		}
